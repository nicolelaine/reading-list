...
const addBook = function (title, author, pages) {
  ...
  return book; //aways need to return!
};
...
button.addEventListener("click", function (e) {
  ...
  // Render to DOM
  addToTable(book); //make sure to pass the parameter of book

  // Clear form
  clearForm();
});

//and again pass the parameter of book here
const addToTable = function (book) {
  const tr = document.createElement("tr");
  const rowContents = `<td>${book.titleValue}</td><td>${book.author}</td><td>${book.pages}</td>`;
